module.exports = app => {
    ...
    const save = (req, res) => {
        ...
        try {
            existsOrError(category.name, 'Category name not found')
        } catch(msg) {
    ...
    const remove = async (req, res) => {
        try {
            existsOrError(req.params.id, 'Category code not found')

            const subcategory = await app.db('categories').where({ parentId: req.params.id })
            notExistsOrError(subcategory, 'This category have subcategories')

            const articles = await app.db('articles').where({ categoryId: req.params.id })
            notExistsOrError(articles, 'This category have articles')
